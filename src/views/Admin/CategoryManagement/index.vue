<template>
    <div>
        ...
        <a-table :columns="columns" :data-source="categories" rowKey="id" :row-selection="rowSelection"
        ...
        <a-modal v-model:visible="createModalVisible" title="Tạo category" okText="Tạo" cancelText="Hủy"
            @ok="handleCreateCategory" @cancel="closeCreateModal">
            <a-form ref="createFormRef" layout="vertical">
                <a-form-item label="Ảnh category">
                ...
                <a-form-item label="Tên" name="name" required>
                    <a-input v-model:value="createForm.name" placeholder="Nhập tên category" />
                </a-form-item>

                <a-form-item label="Slug" name="slug" required>
                    <a-input v-model:value="createForm.slug" placeholder="Nhập slug category" />
                </a-form-item>
            </a-form>
        </a-modal>
    </div>
...
const handleDeleteSelected = async () => {
    ...
    try {
        await Promise.all(selectedRowKeys.value.map((id) => CategoryService.deleteCategory(id)))
